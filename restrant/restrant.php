...
$sql = 'SELECT p.id,p.title,p.image,p.body,c.category_name,p.modified  FROM posts p INNER JOIN categories c ON p.category_id = c.id WHERE p.type_id = 3';
$ichiran = mysqli_query($db, $sql) or die(mysqli_error($db));

//typesテーブルのデータを取ってくる
$ss = sprintf('SELECT * FROM `types` WHERE id=3');
